...
**Stack:** Vue 3 (Composition API, `<script setup>`) + TypeScript + Vite, Pinia, Vue Router, Tailwind CSS v4, `idb` (IndexedDB wrapper), `@vueuse/core`, `@vueuse/motion`, `canvas-confetti`. The PDF keepsake export uses `jspdf` + `html-to-image`, both `import()`-ed on demand (see below).
...
- **Sound** defaults off, with a persistent on/off toggle always visible (`SoundToggle`, wired to `settingsStore.soundEnabled`); the toggle press itself serves as the user gesture required to unlock audio playback under mobile autoplay policies. Nothing plays audio yet — SFX/music are blocked on assets.
