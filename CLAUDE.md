...
- **Planned IndexedDB shape:** a single `memories` object store keyed by memory id (`'1'`–`'4'`) holding `{ photoBlob, capturedAt, puzzleSolved, wishUnlocked }`, plus a `settings` store for `gridSize` / `soundEnabled` / resume state. (Not yet implemented as of this writing — see `src/db/`.)
